...
- **App uptime (DMN-089)**: the `uptime_secs` field on `AppService.GetAppStats`/`StreamAppStats` samples — seconds since the app's current run started. The start-time source differs per kind: docker reads `State.StartedAt` off the inspect call it already makes, process reads `starttime` from `/proc/<pid>/stat` combined with `/proc/stat`'s `btime`, systemd reads `ActiveEnterTimestampMonotonic` (microseconds since boot — no locale-dependent date parsing needed) anchored to the same `btime`. Unset while the app is stopped.
