...
- **`CredentialService`**: the same store, managed by API instead of the CLI — this is how the platform pushes an organization's registry tokens onto a node without ever running `asc auth add` over SSH. `ListCredentials`/`UpsertCredential`/`RemoveCredential` operate on the system store only (`/etc/asc/auth.json`) — a user's own `~/.asc/auth.json` stays exclusively under the CLI. `UpsertCredential` upserts by `(kind, pattern, app)`, same as `GitAuth::add` above, and only ever writes a token credential — an SSH-key credential can be removed through it but never overwritten. `ListCredentials` never returns the token itself.
