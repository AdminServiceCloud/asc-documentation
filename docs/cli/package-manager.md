...
- **`CredentialService`**: the same store, managed by API instead of the CLI — this is how the platform pushes an organization's registry credentials onto a node without ever running `asc auth add` over SSH. `ListCredentials`/`UpsertCredential`/`RemoveCredential` operate on the system store only (`/etc/asc/auth.json`) — a user's own `~/.asc/auth.json` stays exclusively under the CLI. `UpsertCredential` upserts by `(kind, pattern, app)`, same as `GitAuth::add` above, and now takes either secret shape: a token, or raw PEM private-key bytes. For a key, the daemon writes it to a 0600 file it owns under `/etc/asc/ssh-keys` and registers a `Method::SshKey` pointing at it — the same shape `asc auth add --ssh-key` produces by hand. Replacing or removing that credential deletes the file too. `ListCredentials` never returns the secret itself, key file included. **Ownership marker** (`managed_by`): `UpsertCredentialRequest.managed_by` lets a caller stamp its own entries (the platform uses `"platform"`), and `RemoveCredentialRequest.managed_by`, when set, narrows a removal to entries carrying that exact marker — a platform push can list its own credentials, diff them against what it just sent, and remove the leftovers without ever touching an entry the operator added by hand through `asc auth add`. `asc auth list` prints `managed_by=<value>` next to any entry that carries one.
...
**Detecting every installation method a repository supports** (DMN-106): a repository with no `asc.yaml`/`asc.stack.yaml` used to fail `InspectPackage` outright. It now reports back `PACKAGE_KIND_UNSPECIFIED` plus every install method detected in the package directory — ASC's own manifest/stack, a docker compose file, a bare Dockerfile, a Swarm stack, Kubernetes manifests, or a Helm chart — each with the files that triggered it, whether it's installable today, and a plain-English reason when it isn't. A compose file with a `deploy:` block reports both `DOCKER_COMPOSE` and `SWARM`, since it's one file usable two ways. The filesystem walk this needs is bounded (depth 3, ~5,000 entries, skipping `.git`/`node_modules`/`vendor`/`target`/`dist`), and compose itself is parsed permissively rather than with the strict, unknown-field-rejecting parser `asc.yaml` uses — compose is a large foreign format, and a feature this daemon doesn't model must not turn "detected" into "failed to parse". The same change also fixes a private-repository clone surfacing as a bare internal error over gRPC: `InspectPackageResponse.auth_required`/`InstallAppResponse.auth_required` now carry the URL, its normalized pattern, and which transport (https/ssh) authorizes it, the same non-error shape `license_required`/`requirements_not_met` already use.

**Installing straight from a bare Dockerfile** (DMN-107): a repository with a `Dockerfile` and nothing else installs as a normal `type: docker` app — `asc install <url> --method dockerfile` synthesizes a manifest and settings in memory (`EXPOSE` → published ports, `VOLUME` → volumes, paired host/container exactly like a hand-written `asc.settings.yaml`) instead of reading them from the repository, and everything downstream of that (license/resource checks, provisioning) is unchanged. Nothing synthesized is ever written to the repository, so every later reader — a settings-drift recreate, `asc app upgrade`, disk usage, ports, `asc app clone` — re-synthesizes the same manifest from the install method recorded in the app's own metadata rather than failing outright on a missing `asc.yaml`.

**Installing from a bare `docker-compose.yml`** (DMN-108/DMN-109): a repository with a compose file and nothing else installs as a `docker compose` project — no settings, no quota, no requirements check, since there is no manifest at all here. This one exception to "Docker only through bollard" is deliberate and documented: reimplementing compose's interpolation, `env_file`, `extends`, profiles, override files and healthcheck-aware `depends_on` over the Engine API would be a project of its own, so the `docker compose` CLI plugin (v2) is shelled out to instead, pointed at the same configured socket bollard uses everywhere else. The plugin is optional and probed once; missing it leaves compose a "found, not installable" method like Swarm/Kubernetes. Installing provisions with `docker compose create` (containers exist, not started, the same contract as a normal Docker app); `stop` is `stop`, never `down`, so the app's containers and volumes survive it; ports are read from the compose file so a stopped app still reports them; exec/attach are refused (no single container to address) while logs work, already multiplexed by service. The single most load-bearing check is refusing any service that bind-mounts a host path outside the cloned repository, before a container is ever created from it.
...
DMN-003, DMN-018, DMN-038, DMN-040, DMN-045, DMN-046, DMN-047, DMN-048, DMN-059, DMN-084, DMN-096, DMN-097, DMN-098, DMN-099, REG-001, REG-002, REG-005, BE-002, BE-003, BE-028, BE-029, BE-030, NODE-031, NODE-032, NODE-033, FE-090, FE-091 in [ROADMAP.md](https://github.com/AdminServiceCloud/asc-platform/blob/main/ROADMAP.md); GRW-011 in [ROADMAP-GROWTH.md](https://github.com/AdminServiceCloud/asc-platform/blob/main/ROADMAP-GROWTH.md).
