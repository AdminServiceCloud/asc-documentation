...
- **`asc ports [<имя>]`** / **`asc app ports [<имя>]`** (DMN-049): опубликованные порты host==container приложения с транспортом (`27015/tcp`, `27015/udp` или `27015/tcp+udp`, когда оба на одном порту), взятые из настроек `type: ports` — поэтому **остановленное** приложение всё равно показывает порты, которые оно займёт при следующем старте. Без имени — таблица всех видимых приложений с их портами (root видит приложения всех пользователей). **`AppService.ListAppPorts`** (DMN-079) — тот же список по gRPC, для платформы: docker-приложения возвращают свои порты, остальные виды — пустой список.
