...
- Подстановка разрешает `${VAR}` из всего вычисленного окружения приложения — значений настроек с ключами `env:` **и** любых записей `$env` от подключённой группы Environment (см. выше), так что команда запуска может ссылаться и на переменную, доставленную группой — если та попала в `settings.json` до (пере)запуска, который пересчитывает команду. Предпросмотр вычисленной команды в UI платформы — следующий инкремент ([🌱 environments](https://github.com/AdminServiceCloud/asc-platform/blob/main/docs/features/environments.md)).
...
**Детект всех способов установки репозитория** (DMN-106): раньше репозиторий без `asc.yaml`/`asc.stack.yaml` просто ронял `InspectPackage`. Теперь он возвращает `PACKAGE_KIND_UNSPECIFIED` плюс все найденные в каталоге пакета способы установки — собственный манифест/стек ASC, compose-файл, голый Dockerfile, Swarm-стек, Kubernetes-манифесты или Helm-чарт — каждый со списком файлов-триггеров, флагом «устанавливается ли уже сегодня» и простой английской причиной, если нет. Compose-файл с блоком `deploy:` отдаёт сразу оба способа, `DOCKER_COMPOSE` и `SWARM`, — это один файл, применимый двумя способами. Обход файловой системы для этого ограничен (глубина 3, ~5000 записей, пропуск `.git`/`node_modules`/`vendor`/`target`/`dist`), а сам compose парсится разрешающе, а не строгим парсером с отказом на неизвестных полях, как `asc.yaml`, — compose это большой чужой формат, и то, чего демон не моделирует, не должно превращать «обнаружено» в «не удалось разобрать». Это же изменение чинит приватный репозиторий, всплывавший по gRPC голой внутренней ошибкой: `InspectPackageResponse.auth_required`/`InstallAppResponse.auth_required` теперь несут URL, его нормализованный шаблон и транспорт (https/ssh), который его авторизует, — тот же неаварийный формат, что уже есть у `license_required`/`requirements_not_met`.

**Установка прямо из голого Dockerfile** (DMN-107): репозиторий с одним `Dockerfile` и без ничего другого ставится как обычное `type: docker`-приложение — `asc install <url> --method dockerfile` синтезирует манифест и настройки в памяти (`EXPOSE` → опубликованные порты, `VOLUME` → тома, пара хост/контейнер точно как в написанном вручную `asc.settings.yaml`) вместо чтения их из репозитория, а всё, что идёт дальше (проверки лицензии/ресурсов, провижининг), не меняется. Ничего синтезированное никогда не записывается в репозиторий, поэтому каждый последующий читатель — пересоздание при дрейфе настроек, `asc app upgrade`, дисковое использование, порты, `asc app clone` — пересинтезирует тот же манифест из способа установки, записанного в метаданных самого приложения, а не падает на отсутствующем `asc.yaml`.

**Установка из голого `docker-compose.yml`** (DMN-108/DMN-109): репозиторий с compose-файлом и без ничего другого ставится как `docker compose`-проект — ни настроек, ни квоты, ни проверки требований, потому что манифеста тут вообще нет. Это единственное осознанное и задокументированное исключение из правила «Docker только через bollard»: переписывать интерполяцию, `env_file`, `extends`, профили, override-файлы и `depends_on` с ожиданием healthcheck поверх Engine API — отдельный проект работы, поэтому вместо этого вызывается CLI-плагин `docker compose` (v2), направленный на тот же настроенный сокет, что bollard использует везде. Плагин опционален и пробуется один раз; без него compose остаётся способом «найдено, не устанавливается», как Swarm/Kubernetes. Установка провижинит через `docker compose create` (контейнеры существуют, но не запущены — тот же контракт, что у обычного Docker-приложения); «стоп» — это `stop`, никогда не `down`, поэтому контейнеры и тома приложения переживают его; порты читаются из compose-файла, поэтому остановленное приложение всё равно их сообщает; exec/attach отклоняются (нет одного контейнера для адресации), а логи работают, уже мультиплексированные по сервисам. Самая важная проверка — отказ любому сервису, который монтирует хостовый путь за пределами склонированного репозитория, ещё до создания хоть одного контейнера.
