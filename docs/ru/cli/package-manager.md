...
- **`CredentialService`**: тот же стор, только под управлением по API вместо CLI — так платформа выкладывает токены реестров организации на ноду, не запуская `asc auth add` по SSH. `ListCredentials`/`UpsertCredential`/`RemoveCredential` работают только с системным стором (`/etc/asc/auth.json`) — собственный `~/.asc/auth.json` пользователя остаётся исключительно под CLI. `UpsertCredential` апсертит по `(kind, pattern, app)`, как и `GitAuth::add` выше, и всегда пишет только токен-запись — SSH-ключевую запись через него можно удалить, но не перезаписать. `ListCredentials` никогда не возвращает сам токен.
