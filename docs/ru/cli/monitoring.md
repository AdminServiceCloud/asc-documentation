...
- **Аптайм приложения (DMN-089)**: поле `uptime_secs` в сэмплах `AppService.GetAppStats`/`StreamAppStats` — секунды с момента текущего запуска. Источник времени старта свой для каждого вида: docker берёт `State.StartedAt` из уже выполняемого `inspect_container`, process — поле `starttime` из `/proc/<pid>/stat` в сочетании с `btime` из `/proc/stat`, systemd — `ActiveEnterTimestampMonotonic` (в микросекундах с загрузки, без разбора локализованной даты) с той же привязкой к `btime`. Поле не выставляется, пока приложение не запущено.
